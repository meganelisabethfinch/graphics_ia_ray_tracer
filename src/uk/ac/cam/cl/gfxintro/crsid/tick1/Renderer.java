...
public class Renderer {
	...
	protected ColorRGB trace(Scene scene, Ray ray, int bouncesLeft) {
		...
		if (bouncesLeft == 0 || reflectivity == 0) {
			...
			Vector3 R = ray.getDirection().scale(-1.0).reflectIn(N.normalised()).normalised();
	...
	private ColorRGB illuminate(Scene scene, SceneObject object, Vector3 P, Vector3 N, Vector3 O) {
		...
		for (int i = 0; i < pointLights.size(); i++) {
			...
			Vector3 R = (L.reflectIn(N.normalised())).normalised();

			// TODO: Calculate ColorRGB diffuse and ColorRGB specular terms
			ColorRGB diffuse = C_diff.scale(k_d).scale(I).scale(Math.max(0, L.dot(N.normalised())));
			...
			// let d be the distance to the closest intersection
			double d = scene.findClosestIntersection(shadowRay).getDistance();
			// d is positive infinity if no such intersection exists

			// if d is less than distanceToLight, there is another object in the way
